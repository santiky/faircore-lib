...
var MAX_BLOCK_SIZE = 1500000;

// Minimum amount for an output for it not to be considered a dust output
Transaction.DUST_AMOUNT = 0.008 * 1e8; // Jan 2018, dynamic chain param

// Margin of error to allow fees in the vecinity of the expected value but doesn't allow a big difference
Transaction.FEE_SECURITY_MARGIN = 150;

// max amount of satoshis in circulation
Transaction.MAX_MONEY = 53193831.467966 * 1e8;
...
Transaction.FEE_PER_KB = 800000; // Jan 2018, dynamic chain param
...
//  var fee = Math.ceil(size / 1000) * (feePerKb || Transaction.FEE_PER_KB); //  OLD, Math.ceil devuelve siempre 1 ¿?
  var fee = ( size / 1000 ) * (feePerKb || Transaction.FEE_PER_KB);
  if (amountAvailable > fee) {
    size += Transaction.CHANGE_OUTPUT_MAX_SIZE;
  }
  return (size / 1000) * (feePerKb || Transaction.FEE_PER_KB);
};
